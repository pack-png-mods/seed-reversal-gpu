...
void setup_gpu_node(GPU_Node* node, int gpu) {
    ...
    CHECK_GPU_ERR(cudaMallocManaged(&node->num_rand_starts, sizeof(*node->num_rand_starts)));
    CHECK_GPU_ERR(cudaMallocManaged(&node->rand_starts, (sizeof(Random)*WORK_UNIT_SIZE*10000)));
}
