...
#define WATERFALL_X 4
#define WATERFALL_Y 76
#define WATERFALL_Z 10

#define TREE_X (WATERFALL_X - 3)
#define TREE_Z (WATERFALL_Z + 3)
...
__device__ inline bool addTreeFlags(int* flags, int x, int z, int height) {
    ...
    *flags |= ((x >= WATERFALL_X + 3 && x <= WATERFALL_X + 5) && (z <= WATERFALL_Z - 6 && z >= WATERFALL_Z - 9) && (height == 4 || height == 5)) << 1;
...
void setup_gpu_node(GPU_Node* node, int gpu) {
    ...
    CHECK_GPU_ERR(cudaMallocHost(&node->seeds, (1LL << 20))); // approx 1MB
